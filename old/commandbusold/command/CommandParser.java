...
{
    private static final char FLAG_MARKER = '=';
    ...
    private String readCommand()
    {
        ...
        while (pos < in.length() && read() != FLAG_MARKER)
        {
            if (read() == ' ')
            {
                lastSpace = pos;
    ...
    {
        int start = pos;
        while (hasNext() && read() != FLAG_MARKER)
        {
            pos++;
        }
    ...
    {
        boolean quoted = read() == QUOTE_MARKER;
        int start = quoted ? ++pos : pos, end = start;

        while (pos < in.length() && ((quoted && read() != QUOTE_MARKER) || (!quoted && read() != FLAG_MARKER)))
        {
            if (read() == ' ')
            {
                end = pos;
